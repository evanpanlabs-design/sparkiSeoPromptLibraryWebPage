# Agent Prompt Templates (react/prompt.py)

**File:** `src/agent/react/prompt.py`  
**Purpose:** Alternative/template prompt versions used by the `react/` module. These are older/original versions that use dynamic `{placeholder}` substitution. The `nodes/` directory versions are the active V3.2 versions.
...
## 1. THINK System Prompt Template

```python
THINK_SYSTEM_PROMPT = """你是一个专业的 AI 内容助理，代号 Sparki。
...
**Intent types:** STATUS_QUERY | TASK_EXECUTION | SEARCH | MULTI_STEP | CLARIFICATION | CHITCHAT

---

## 2. PLAN System Prompt Template

```python
PLAN_SYSTEM_PROMPT = """你是一个任务规划专家，负责为 Sparki Agent 制定具体的行动计划。
...
**Response format:** `action: {...}` or `reply: <text>`

---

## 3. OBSERVE System Prompt Template
...
**Response format:** `continue: true|false` + optional `next_thought: ...`

---

## 4. FINAL REPLY System Prompt Template

```python
FINAL_REPLY_SYSTEM_PROMPT = """你是一个友好的 AI 助手，负责将 Sparki Agent 的工作结果整理成最终回复。
...
**Response format:** Plain text (no structured format)